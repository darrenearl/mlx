...
void QuantizedMatmul::eval_gpu(const std::vector<array>& inputs, array& out) {
  ...
  auto make_contiguous = [&](const array& a, bool check) -> array {
    if (check) return a;
    auto c = contiguous(a, false, s);
    eval(c);
    return c;
  };

  array x_contig      = make_contiguous(inputs[0], inputs[0].flags().row_contiguous);
  array w_contig      = make_contiguous(inputs[1], is_last_2_dims_row_contiguous(inputs[1]));
  array scales_contig = make_contiguous(inputs[2], is_last_2_dims_row_contiguous(inputs[2]));

  std::optional<array> biases_contig;
  if (inputs.size() > 3) {
    biases_contig = make_contiguous(inputs[3], is_last_2_dims_row_contiguous(inputs[3]));
  }

  const array& x = x_contig;
  const array& w = w_contig;
  const array& scales = scales_contig;
  const std::optional<array>& biases = biases_contig;

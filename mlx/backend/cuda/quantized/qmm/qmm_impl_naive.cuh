...
void qmm_impl_naive(
  ...
  // ↓↓↓ 新增这一行 ↓↓↓
  if (rhs_indices.has_value()) broadcast_b = false;
  // ↑↑↑ 新增结束 ↑↑↑
    ...
    dispatch_k<KMajor>(k % group_size != 0, tag, [&]<bool has_k_residue>() {
      dispatch_element_types(out.dtype(), tag, [&]<typename Element>() {
        dispatch_quant_types<Element>(
            bits, group_size, mode, tag,
            [&]<typename Quant, typename Scale, int group_size>() {
              encoder.set_input_array(x);
              encoder.set_input_array(w);
              encoder.set_input_array(scales);
              if (biases) encoder.set_input_array(*biases);
              if (lhs_indices) encoder.set_input_array(*lhs_indices);
              if (rhs_indices) encoder.set_input_array(*rhs_indices);
              ...
              cutlass_gemm::qmm_naive<TileM, KMajor, sm80.value, has_k_residue>(
                  ...
                  broadcast_b,
                  cute::Int<group_size>{},
                  [&](auto* kernel, dim3 num_blocks, dim3 block_dims,
                      uint32_t smem_bytes, void** args) {
